...
## Índice — Escopo e Tipo

**Escopo:** `v1` = aluno (momentum-aluno.html, foco atual) · `v2` = PT dashboard
(momentum-dashboard-v5.html / momentum-pt-dashboard.html, adiado) · `parcial` = regra
com componente em ambos, ver nota na própria RN.

**Tipo:** `workflow` = máquina de estado / trigger de ação · `validação` = bloqueio
ou rejeição de input · `notificação` = canal/mensagem sem mudança de estado ·
`cálculo` = fórmula ou lógica de interpretação de dado.

| RN | Regra | Escopo | Tipo |
|----|-------|--------|------|
| 01 | Divisão prioritária pulada | v2 | workflow |
| 02 | Divisão não prioritária pulada | v2 | workflow |
| 03 | Sessão abandonada no meio | v1 | workflow |
| 04 | Mesociclo encerrado | v1 | workflow |
| 05 | Mesociclo encerrando em breve | v2 | notificação |
| 06 | Exercício extra adicionado | v1 | workflow |
| 07 | Aluno sem treinar há X dias | v2 | notificação |
| 08 | Carga acima do limite de segurança | v2 | validação |
| 09 | Volume semanal acima do teto | parcial | validação |
| 10 | Entrada absurda de carga | v1 | validação |
| 11 | Edição de sessão pelo PT | v2 | workflow |
| 12 | Detecção de PR — confirmação dupla | v1 | workflow |
| 13 | Progressão semiautomática | v2 | workflow |
| 14 | Lembrete de treino | v1 | notificação |
| 15 | Novo mesociclo disponível | v1 | notificação |
| 16 | Comentário do PT em progressão | v2 | notificação |
| 17 | Canais e preferências de notificação | v2 | notificação |
| 18 | Tela de transição — fim de mesociclo | v1 | workflow |
| 19 | Cálculo do Índice de Carga | v1 | cálculo |
| 20 | Ritmo de Adaptação | v1 | cálculo |
| 21 | Dimensões por sessão e radar | v1 | cálculo |
| 21b | Scores do aluno (0–10) | v1 | cálculo |
| 22 | Competência Técnica — propriedade do exercício | v1 | cálculo |
| 23 | Modelos de progressão intra-mesociclo | v2 | cálculo |
| 24 | Interpretação dimensional por nível | v2 | cálculo |
| 25 | Sistema PSE | v1 | cálculo |
| 26 | Detecção automática de padrões (chips) | v2 | workflow |
